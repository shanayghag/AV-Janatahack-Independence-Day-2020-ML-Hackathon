...
def val(model, device, val_dataloader, criterion):
    ...
    for step, batch in enumerate(val_dataloader):
        ...
        with torch.no_grad():
            # forward pass
            logits, _ = model(input_ids=b_input_ids, attention_mask=b_attention_mask)
...
def get_tf_embed(model, device, dataloader, set_type=None):
    embeddings = []
    labels = []
    for step, batch in tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        with torch.no_grad():
            _, embedding = model(input_ids=b_input_ids, attention_mask=b_attention_mask)
...
def train_val_grad_boost(
    ...
    logits = model.predict(val_embeddings)
    val_micro_f1_score = f1_score(val_labels, logits, average='micro')
